...
?>
<div class="" id="left-theme-list" rel="left-theme-list">
            ...
            <table class="searchContent">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a class="btnLook" href="<?=Url::to(['customer/left-customer-list', 'search' => 1])?>" lookupgroup="customer">查找带回</a>
                        <?php endif;?>
                    </td>
                    <td>
                        颜色:
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <?php if(\Yii::$app->user->can('theme/edit-left-theme')):?>
            <li><a class="add" href="<?=Url::to(['theme/edit-left-theme', 'customer_id' => ArrayHelper::getValue($other, 'customer_id'), 'customer_name' => urlencode(ArrayHelper::getValue($params, 'customer-name',''))])?>" target="navTab"><span>添加</span></a></li>
            <?php endif;?>

            <?php if(\Yii::$app->user->can('theme/delete-left-theme')):?>
            <li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids[]" href="<?=Url::to(['theme/delete-left-theme'])?>" class="delete"><span>批量删除</span></a></li>
            <?php endif;?>
        </ul>
    </div>
    <table class="table" width="1200" layoutH="138">
        ...
        <?php foreach($dataList as $key => $data):?>
            <tr target="card-id" rel="<?=$data['id']?>" class="cnt">
                ...
                <td>
                    <?php if(\Yii::$app->user->can('theme/delete-left-theme')):?>
                    <a title="删除" target="ajaxTodo" href="<?=Url::to(['theme/delete-left-theme','ids' => $data['id']])?>" class="btnDel">删除</a>
                    <?php endif;?>

                    <?php if(\Yii::$app->user->can('theme/edit-left-theme')):?>
                    <a title="编辑" target="navTab" href="<?=Url::to(['theme/edit-left-theme','id' => $data['id']])?>" class="btnEdit dblclick">编辑</a>
                    <?php endif;?>

                    <?php if(\Yii::$app->user->can('meal/meal-list')):?>
                        <a title="套餐列表" target="navTab" rel="meal-list" href="<?=Url::to(['meal/meal-list', 'notMore' => 1, 'other' => ['left-theme_id' => $data['id'],'customer_id' => $data['customer']['id']],'left-theme-name' => urlencode($data['name']),'customer-name' => urlencode($data['customer']['name'])])?>" class="btnView">套餐列表</a>
                    <?php endif;?>
                </td>
                <?php endif;?>
            </tr>
    ...
    <div class="panelBar">
        ...
        <div class="pagination" rel='left-theme-list' targetType="<?=$search?'dialog':'navTab'?>" totalCount="<?=$dataCount?>" numPerPage="<?=$prePage?>" pageNumShown="10" currentPage="<?=$page?>"></div>
    </div>
</div>
</div>
